...
public class ColorConverter {
    ...
    public enum YCbCrColorSpace {ITU_BT_601,ITU_BT_709_HDTV}
    ...
    public static int[] YUVtoRGB(double y, double u, double v){
        ...
        r = (y + 0.000 * u + 1.140 * v) * 255;
        g = (y - 0.396 * u - 0.581 * v) * 255;
        b = (y + 2.029 * u + 0.000 * v) * 255;
    ...
    public static int[] YCbCrtoRGB(double y, double cb, double cr, YCbCrColorSpace colorSpace){
        ...
        if (colorSpace == YCbCrColorSpace.ITU_BT_601) {
            r = (y + 0.000 * cb + 1.403 * cr) * 255;
            g = (y - 0.344 * cb - 0.714 * cr) * 255;
            b = (y + 1.773 * cb + 0.000 * cr) * 255;
        }
        else{
            r = (y + 0.000 * cb + 1.5701 * cr) * 255;
            g = (y - 0.1870 * cb - 0.4664 * cr) * 255;
            b = (y + 1.8556 * cb + 0.000 * cr) * 255;
        }
    ...
    public static int[] XYZtoRGB(double x, double y, double z){
        ...
        if ( r > 0.0031308 )
            r = 1.055f * Math.pow(r, 0.4166f) - 0.055f;
        else
            r = 12.92f * r;
        
        if ( g > 0.0031308 )
            g = 1.055f * Math.pow(g, 0.4166f) - 0.055f;
        else
            g = 12.92f * g;
        
        if ( b > 0.0031308 )
            b = 1.055f * Math.pow(b, 0.4166f) - 0.055f;
        else
            b = 12.92f * b;
    ...
    public static double[] RGBtoC1C2C3(int r, int g, int b){
        ...
        c[0] = Math.atan(r / Math.max(g, b));
        c[1] = Math.atan(g / Math.max(r, b));
        c[2] = Math.atan(b / Math.max(r, g));
